...
def add_flight(destinations):
    ...
    save_data(destinations, os.getenv('DESTINATIONS_FILE', 'destinations.json'))  # Используем переменную окружения DESTINATIONS_FILE
...
def main():
    ...
    filename = os.getenv('DESTINATIONS_FILE', 'destinations.json')

    # Проверка, была ли переменная окружения установлена
    if filename == 'destinations.json':
        print("Переменная окружения не установлена. Используется файл по умолчанию 'destinations.json'.")
